...
module mkWishboneSlaveXactor_32_32(CLK,
				   RST_N,
  ...
  // register m_rg_ack
  reg m_rg_ack;
  wire m_rg_ack$D_IN, m_rg_ack$EN;
  ...
  // register m_rg_ack
  assign m_rg_ack$D_IN = m_io_rsp$EMPTY_N && CYC_I ;
  assign m_rg_ack$EN = 1'd1 ;

  // submodule m_io_req
  assign m_io_req$D_IN = { WE_I, SEL_I, ADR_I, DAT_I } ;
  assign m_io_req$ENQ = m_io_req$FULL_N && CYC_I && STB_I ;
  ...
  assign m_io_rsp$DEQ = m_io_rsp$EMPTY_N && CYC_I ;
  ...
  begin
    if (RST_N == `BSV_RESET_VALUE)
      begin
        m_rg_ack <= `BSV_ASSIGNMENT_DELAY 1'd0;
      end
    else
      begin
        if (m_rg_ack$EN) m_rg_ack <= `BSV_ASSIGNMENT_DELAY m_rg_ack$D_IN;
      end
  end
...
endmodule  // mkWishboneSlaveXactor_32_32
