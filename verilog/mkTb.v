...
module mkTb(CLK,
  ...
  assign m_dat$EN = master$RDY_server_request_put ;
  ...
  assign master$EN_server_request_put = master$RDY_server_request_put ;
  ...
  begin
    #0;
    if (RST_N != `BSV_RESET_VALUE)
      if (master$RDY_server_request_put)
	begin
	  v__h536 = $time;
	  #0;
	end
    if (RST_N != `BSV_RESET_VALUE)
      if (master$RDY_server_request_put)
	$display("%t master request %h", v__h536, m_dat);
    if (RST_N != `BSV_RESET_VALUE)
    ...
    if (RST_N != `BSV_RESET_VALUE)
      if (slave$RDY_client_response_put)
	$display("%t         slave response %h", v__h694, s_dat);
    if (RST_N != `BSV_RESET_VALUE)
